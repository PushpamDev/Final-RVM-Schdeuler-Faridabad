...
export default function FreeSlots() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      .then(([facultyRes, skillsRes]) =>
        Promise.all([facultyRes.json(), skillsRes.json()])
      )
      .then(([facultiesData, skillsData]) => {
  ...
  const handleSearch = async () => {
    ...
    try {
      setLoading(true);
      const response = await fetch(`/api/free-slots?${params.toString()}`);

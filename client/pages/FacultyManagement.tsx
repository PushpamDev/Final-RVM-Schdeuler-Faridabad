  ...
  useEffect(() => {
    ...
    if (faculty && open) {
      const fetchAvailability = async () => {
        try {
          const response = await fetch(`/api/availability/faculty/${faculty.id}`);
  ...
  const handleSaveFaculty = async (data: FacultyFormData) => {
    ...
    try {
      ...
      if (selectedFaculty) {
        ...
        facultyResponse = await fetch(`/api/faculty/${selectedFaculty.id}`, {
          method: "PUT",
        ...
        // Add new faculty
        facultyResponse = await fetch(`/api/faculty`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(facultyPayload),
        });
